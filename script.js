...
function initCardTilt() {
  ...
  const cards = $$('.sim-card, .evento-card, .sim-spec-card');
...
// ─────────────────────────────────────────────────────
// 18b. CARD SPOTLIGHT (glow que sigue al mouse)
// Aplica a proceso, specs del simulador y programas.
// ─────────────────────────────────────────────────────
function initCardSpotlight() {
  if (window.matchMedia('(hover: none)').matches) return;

  const cards = $$('.step-card, .sim-spec-card, .evento-card');
...
// ─────────────────────────────────────────────────────
// 18c. PROCESO — paso activo al centrarse + runner en la línea
// ─────────────────────────────────────────────────────
function initProcesoActive() {
  const section = $('#proceso');
  const steps = $$('.proceso-step');
  const runner = $('#procesoRunner');
  if (!section || !steps.length) return;

  // Resalta el paso que está centrado en pantalla
  ...
  // El puntito recorre la línea según el avance del scroll en la sección
  let ticking = false;
...
// ─────────────────────────────────────────────────────
// 18d. SIMULADOR — parallax de imagen + spotlight en hover
// ─────────────────────────────────────────────────────
function initSimHeroInteractive() {
  if (window.matchMedia('(hover: none)').matches) return;

  const heroBox = $('.sim-hero');
  const img = $('.sim-hero-image');
  const spotlight = $('.sim-hero-spotlight');
  if (!heroBox) return;
...
// ─────────────────────────────────────────────────────
// 18e. SIMULADOR — specs con conteo animado (6DOF, 240°, 20NM, 4K)
// ─────────────────────────────────────────────────────
function initSimCounters() {
  const counters = $$('.sim-counter');
  if (!counters.length) return;
  ...
  const observer = new IntersectionObserver(entries => {
    entries.forEach(entry => {
      if (!entry.isIntersecting) return;
      animateSimCounter(entry.target);
      observer.unobserve(entry.target);
    });
  }, { threshold: 0.5 });
...
if (!window.matchMedia('(hover: none)').matches) {
  ...
  // Nuevas interacciones: proceso, simuladores y programas
  initCardSpotlight();
  initProcesoActive();
  initSimHeroInteractive();
  initSimCounters();
}
...
/* ==========================================
   ...
   (el tilt real de .evento-card ahora vive en
   initCardTilt() más arriba, junto con .sim-spec-card,
   para evitar dos listeners peleando por el transform)
========================================== */
